...
- Tratamento de Dados Ausentes: Identificação de nulos ou vazios na coluna product_category_name, preenchendo-os com a string "sem categoria".
- Padronização: Aplicação de strip(), lower() e limpeza de caracteres especiais via Expressões Regulares (Regex) para normalizar os nomes das categorias.
- Regra de Negócio: Aplicação de lógica condicional para verificar pedidos com datas de entrega ausentes, confirmando a hipótese de que esses casos estão atrelados a pedidos com status de "canceled".
